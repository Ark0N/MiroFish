...
Optimizations:
1. Uses graph search to enrich node information
2. Optimized prompts for generating detailed personas
...
class OasisProfileGenerator:
    ...
    Converts graph entities into OASIS simulation Agent Profiles
    
    Optimization features:
    1. Uses graph search for richer context
    ...
    # Group/organization entity types (require group representative persona generation)
    GROUP_ENTITY_TYPES = [
    ...
    def __init__(
        ...
        try:
            GraphitiManager.get_instance()
            self._graphiti_available = True
        except Exception as e:
            logger.warning(f"Graphiti client not available: {e}")
    ...
    def _search_graph_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Graphiti search to get enriched information about an entity.
        ...
        if not self.graph_id:
            logger.debug(f"Skipping graph search: no graph_id set")
            return results
        ...
        try:
            ...
            search_results = run_async(graphiti.search(
            ...
            logger.info(f"Graph search complete: {entity_name}, found {len(results['facts'])} facts")
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        3. Rich information from graph search
        """
        context_parts = []
        ...
        # 4. Use graph search to get enriched information
    ...
    def _generate_profile_with_llm(
        ...
        - Individual entity: generates specific character settings
        - Group/organization entity: generates representative account settings
        """
        ...
        # Retry multiple times until success or max retries reached
        max_attempts = 3
        last_error = None
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt."""
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for opinion simulation, maximally faithful to known real-world information. You must return valid JSON format, all string values must not contain unescaped newlines. Use English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for an entity, maximally faithful to known real-world information.
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        """Build detailed persona prompt for group/organization entities."""
        
        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "None"
        context_str = context[:3000] if context else "No additional context"
        
        return f"""Generate a detailed social media account profile for an organization/group entity, maximally faithful to known real-world information.
...
2. persona: Detailed account profile description (2000 words of plain text), including:
   - Organization basic info (official name, nature, establishment background, main functions)
...
- Organization account communication must match its identity and positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            graph_id: Graph ID, used for graph search to get richer context
        ...
        logger.info(f"Starting agent persona generation - {total} entities, parallelism: {parallel_count}")
