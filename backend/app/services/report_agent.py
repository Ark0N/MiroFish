...
Uses ReACT pattern for simulation report generation with graph retrieval.
...
class ReportLogger:
    ...
    Generates agent_log.jsonl in the report folder, recording each detailed action step.
    ...
    def log_section_content(
        ...
        """Log section content generation complete (content only, does not mean full section complete)."""
        self.log(
...
系统会Executing tool并把结果返回给你。你不需要也不能自己编写工具返回结果。
...
REACT_TOOL_LIMIT_MSG = (
    "tool call count已达上限（{tool_calls_count}/{max_tool_calls}），不能再调用工具。"
...
class ReportAgent:
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        Returns:
            工具执行结果（文本格式）
        """
        logger.info(f"Executing tool: {tool_name}, 参数: {parameters}")
    ...
    def plan_outline(
        ...
        Returns:
            ReportOutline: 报告大纲
        """
        logger.info("开始Planning report outline...")
        ...
        try:
            ...
            logger.info(f"Outline planning complete: {len(sections)} 个章节")
            return outline
            
        except Exception as e:
            logger.error(f"大纲规划失败: {str(e)}")
            # 返回Default outline（3个章节，作为fallback）
    ...
    def _generate_section_react(
        ...
        min_tool_calls = 3  # 最少tool call count
        ...
        if response is None:
            ...
            final_answer = f"（本Section generation failed：LLM 返回空响应，请稍后重试）"
    ...
    def generate_report(
        ...
        """
        Generate complete report（分章节实时输出）
        
        每个Section generation complete后立即保存到文件夹，不需要等待整个报告完成。
        文件结构：
        reports/{report_id}/
            meta.json       - 报告元信息
            outline.json    - 报告大纲
        ...
        try:
            ...
            ReportManager.update_progress(
                report_id, "planning", 5, "开始Planning report outline...",
                completed_sections=[]
            )
            ...
            if progress_callback:
                progress_callback("planning", 0, "开始Planning report outline...")
            ...
            ReportManager.update_progress(
                report_id, "planning", 15, f"Outline planning complete，共{len(outline.sections)}个章节",
                completed_sections=[]
            )
            ReportManager.save_report(report)
...
class ReportManager:
    ...
    def save_section(
        ...
        在每个Section generation complete后立即调用，实现分章节输出
