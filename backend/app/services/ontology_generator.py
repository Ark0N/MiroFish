...
Analyzes text content and generates entity and relationship type definitions for social simulation.
"""
...
- We need to simulate the reactions of various parties in opinion events and the information propagation paths

Therefore, **entities must be real-world subjects that can actually speak and interact on social media**:
...
A. **Fallback types (must include, placed as last 2 in list)**:
   - `Person`: Fallback type for any individual. When a person doesn't belong to other more specific person types, classify here.
   - `Organization`: Fallback type for any organization. When an organization doesn't belong to other more specific types, classify here.

B. **Specific types (8, designed based on text content)**:
   - Design more specific types for the main roles appearing in the text
   - For example: if text involves academic events, you could have `Student`, `Professor`, `University`
   - For example: if text involves business events, you could have `Company`, `CEO`, `Employee`

**Why fallback types are needed**:
- Various people appear in the text, such as "elementary school teachers", "passersby", "anonymous netizens"
...
- Description must clearly explain the difference from the fallback type
...
class OntologyGenerator:
    ...
    def __init__(self, llm_client: Optional[LLMClient] = None):
        self.llm_client = llm_client or LLMClient(cost_phase="ontology")
    ...
    # Maximum text length to send to LLM (50k chars)
    ...
    def _build_user_message(
        ...
        if len(combined_text) > self.MAX_TEXT_LENGTH_FOR_LLM:
            ...
            combined_text += f"\n\n...(original text: {original_length} chars, first {self.MAX_TEXT_LENGTH_FOR_LLM} chars used for ontology analysis)..."
...
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Graphiti limit: max 10 custom entity types, max 10 custom edge types
        MAX_ENTITY_TYPES = 10
        MAX_EDGE_TYPES = 10
    ...
    def generate_python_code(self, ontology: Dict[str, Any]) -> str:
        ...
        code_lines = [
            ...
            'from pydantic import BaseModel  # Graphiti uses Pydantic BaseModel for entity types',
            '',
            '',
