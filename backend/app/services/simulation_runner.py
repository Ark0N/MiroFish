...
class SimulationRunner:
    ...
    # Threading lock for all class-level mutable dicts
    ...
    def get_run_state(cls, simulation_id: str) -> Optional[SimulationRunState]:
        ...
        # Try loading from file（outside lock - file I/O)
    ...
    def _load_run_state(cls, simulation_id: str) -> Optional[SimulationRunState]:
        ...
        try:
            ...
            # Load recent actions (data is already in newest-first order)
    ...
    def _save_run_state(cls, state: SimulationRunState):
        """Save run state to file (atomic write with file locking)."""
    ...
    def start_simulation(
        ...
        max_rounds: int = None,  # Max simulation rounds (optional, truncates overly long simulations)
        ...
        Args:
            ...
            max_rounds: Maximum simulation rounds (optional, truncates overly long simulations)
        ...
        try:
            ...
            # Create main log file to avoid stdout/stderr pipe buffer blocking
            ...
            # Set working directory to simulation directory (databases etc. generated here)
            # Use start_new_session=True to create new process group for os.killpg termination
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group to terminate all related processes on shutdown
            )
            
            # Save file handles and process references（under lock for dict writes)
            ...
            # Start monitor thread（outside lock - thread.start is a long op)
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        try:
            ...
            # Stop graph memory updater (check under lock, call stop outside lock)
            with cls._lock:
            ...
            # Clean up process resources and file handles (grab handles under lock, close outside)
            with cls._lock:
                ...
                for line in f:
                    ...
                    if line:
                        try:
                            ...
                            # If graph memory update enabled, send activities to graph
                            if graph_updater:
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determines if a platform is enabled by checking whether its actions.jsonl file exists
        ...
        # Return False if platform is enabled but not completed
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        """
        Cross-platform termination of process and its children
    ...
    def stop_simulation(cls, simulation_id: str) -> SimulationRunState:
        ...
        # Terminate process (grab ref under lock, terminate outside lock)
        with cls._lock:
            process = cls._processes.get(simulation_id)
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check if there is anything to clean up (snapshot under lock)
        with cls._lock:
        ...
        # Copy dict to avoid modification during iteration (snapshot under lock)
        with cls._lock:
        ...
        # Snapshot and clear file handles under lock, then close outside lock
        with cls._lock:
            stdout_handles = list(cls._stdout_files.items())
            stderr_handles = list(cls._stderr_files.items())
            cls._stdout_files.clear()
            cls._stderr_files.clear()
            cls._processes.clear()
            cls._action_queues.clear()

        for simulation_id, file_handle in stdout_handles:
            try:
                if file_handle:
                    file_handle.close()
            except Exception:
        ...
        for simulation_id, file_handle in stderr_handles:
            try:
                if file_handle:
                    file_handle.close()
            except Exception:
    ...
    def register_cleanup(cls):
        ...
        Called at Flask app startup to ensure all simulation processes are cleaned up on shutdown
        """
        ...
        # In non-debug mode, this env var does not exist but registration is still needed
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None
        
        # In debug mode, register only in reloader child; in non-debug, always register
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Target platform (optional)
                ...
                - None: In dual-platform mode, interview on both platforms and return combined results
    ...
    def interview_agents_batch(
        ...
        Args:
            simulation_id: Simulation ID
            interviews: Interview list, each containing {"agent_id": int, "prompt": str, "platform": str (optional)}
            platform: Default platform (optional, overridden by per-interview platform)
    ...
    def close_simulation_env(
        ...
        Send close environment command to the simulation for graceful exit from wait mode
