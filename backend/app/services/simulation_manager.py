...
using preset scripts + LLM-powered configuration generation.
"""
...
class SimulationManager:
    ...
    # In-memory simulation state cache (class-level, shared by all instances)
    ...
    def _save_simulation_state(self, state: SimulationState):
        """Save simulation state to file (atomic write with file locking)."""
    ...
    def prepare_simulation(
        ...
        try:
            ...
            if progress_callback:
                progress_callback(
                    "reading", 100,
                    f"Done, {len(entities)} entities" + (f"(limited to {max_agents})" if max_agents and filtered.filtered_count > max_agents else ""),
            ...
            # Pass graph_id to enable graph search for richer context
            ...
            profiles = generator.generate_profiles_from_entities(
                ...
                graph_id=state.graph_id,  # Pass graph_id for graph search
            ...
            # Reddit was already saved in real-time during generation; save again for completeness
            if progress_callback:
                progress_callback(
            ...
            if state.enable_twitter:
                # Twitter uses CSV format per OASIS requirements
                generator.save_profiles(
                    profiles=profiles,
            ...
            config_generator = SimulationConfigGenerator(model_name=model_name)
            ...
            # Note: Run scripts stay in backend/scripts/, no longer copied to simulation directory
